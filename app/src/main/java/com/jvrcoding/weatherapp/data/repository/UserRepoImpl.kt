...
class UserRepoImpl(
    ...
    override suspend fun insertUser(user: User): Result<Unit, DataError> {
        val result = dao.insertUser(user.copy(password = user.password.hashPassword()))
        return if(result > 0) {
            Result.Success(Unit)
        } else {
    ...
    override suspend fun getUser(username: String, password: String): Result<User, DataError> {
        val user = dao.getUser(username, password.hashPassword()) ?: return Result.Error(DataError.Local.NO_DATA_FOUND)
